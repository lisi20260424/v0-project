...
import { generateClientSn, precreate } from "@/lib/payment/shouqianba"
...
export async function POST(request: Request) {
  ...
  if (!settings || !settings.enabled) {
    return NextResponse.json({ error: "支付功能尚未开通，请联系管理员" }, { status: 503 })
  }

  const sqbConfig = toShouQianBaConfig(settings)
  if (!sqbConfig) {
    return NextResponse.json({ error: "支付配置不完整，请联系管理员" }, { status: 503 })
  }
  ...
  const totalAmountFen = Math.round(plan.price * 100).toString()
  ...
  try {
    const result = await precreate(sqbConfig, {
      ...
      description: `${planName} - ${user.email ?? user.id}`,
      reflect: order.id,
    })

    providerResponse = result

    if (result.result_code === "200" && result.biz_response?.result_code === "PAY_CREATED") {
      qrCode = result.biz_response.qr_code ?? null
      providerSn = result.biz_response.sn ?? null
    } else {
