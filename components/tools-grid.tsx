    ...
    <section id="tools" className="border-b border-border/60 bg-muted/30">
      <div className="mx-auto max-w-7xl px-4 py-16 md:px-6 md:py-20">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <h2 className="mt-4 text-balance text-3xl font-semibold tracking-tight md:text-4xl">一个账号，解锁所有顶级 AI 能力</h2>
          <p className="mt-3 text-pretty text-muted-foreground">
            统一充值、统一点数、统一计费。无需翻墙，无需多个账号，在国内直接使用全球前沿的 AI 模型。
          </p>
        </div>
                ...
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  {items.map((tool) => {
                    ...
                    return (
                      ...
                      >
                        <div className={cn("pointer-events-none absolute -right-8 -top-8 h-32 w-32 rounded-full bg-gradient-to-br opacity-60 blur-2xl transition-opacity group-hover:opacity-100", tool.accent)} />
                        <div className="relative flex items-start justify-between">
                          <div className={cn("flex h-11 w-11 items-center justify-center rounded-xl bg-gradient-to-br text-foreground ring-1 ring-border", tool.accent)}>
                            <Icon className="h-5 w-5" />
                          </div>
                          <div className="flex items-center gap-2">
                            {tool.tag ? <span className="rounded-full border border-primary/30 bg-primary/5 px-2 py-0.5 text-[10px] font-medium text-primary">{tool.tag}</span> : null}
                        ...
                        <div className="relative mt-4">
                          ...
                          <p className="mt-2 line-clamp-2 text-sm leading-relaxed text-muted-foreground">{tool.desc}</p>
                        </div>
                        <div className="relative mt-4 flex items-center justify-between border-t border-border/60 pt-3 text-xs">
                          ...
                          <span className="font-medium text-foreground">{tool.cost}</span>
                        </div>
                      </Link>
                    )
                  })}
