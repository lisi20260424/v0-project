...
export function ImageGenerator({ models, defaultModelId, prompts = [] }: ImageGeneratorProps) {
  ...
  async function onGenerate() {
    ...
    setLoading(true)
    try {
      await platformAPI.createTask(token, { type: "image", modelId: selected.id, prompt, params: { source: "image-page" } })
  ...
  if (!models.length) {
    return <div className="rounded-2xl border border-dashed border-border bg-card p-10 text-center text-sm text-muted-foreground">暂无可用图像模型。</div>
  }
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
      <section className="rounded-2xl border border-border bg-card p-5">
        ...
        <Textarea id="image-prompt" value={prompt} onChange={(event) => setPrompt(event.target.value)} placeholder="描述你想生成的图像主体、风格、构图和细节。" className="mt-2 min-h-40 resize-none bg-background" />
        {prompts.length > 0 && (
